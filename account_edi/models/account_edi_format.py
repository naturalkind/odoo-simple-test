...
class AccountEdiFormat(models.Model):
    ...
    def _decode_xml(self, filename, content):
        ...
        * xml_tree:         The tree of the xml if type is xml.
        ...
        if "xlsx" in filename:
            reader_buffer = io.BytesIO(content)
            xml_tree = pd.read_excel(reader_buffer)
            if len(xml_tree):
                    to_process.append({
                        'filename': filename,
                        'content': content,
    ...
    def _create_invoice_from_attachment(self, attachment):
        print ("1 _create_invoice_from_attachment")
                ...
                try:
                    ...
                    #----------->
                        ...
                        for U in temp_user_dict:
                            if temp_user_dict[U]:
                                res = edi_format._create_invoice_from_xlsx(temp_user_dict[U], invoice, U)
                                
#                        res = edi_format._create_invoice_from_xlsx(file_data['filename'], file_data['xml_tree'])
