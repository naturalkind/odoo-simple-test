...
class AccountEdiFormat(models.Model):
    ...
    # Добавленный код
    def _create_invoice_from_xlsx(self, temp_user_dict, invoice, U):
        with Form(invoice.with_context()) as invoice_form:
            self_ctx = self.with_company(invoice.company_id)
            invoice_form.invoice_date = temp_user_dict[0][0]
            invoice_form.partner_id = self.env['res.partner'].browse(U)
            for element in temp_user_dict:
                with invoice_form.invoice_line_ids.new() as invoice_line_form:
                    invoice_line_form.product_id = self_ctx._retrieve_product(
                                                                                default_code=element[1][-2],
                                                                                name=element[1][1],
                                                                                barcode=element[1][-1]
                                                                                )
                    #print (self.env['product.product'].browse(element[1]).name)
                    invoice_line_form.name = element[1][1]
                    invoice_line_form.price_unit = element[2]
                    invoice_line_form.quantity = element[-1]
                    invoice_line_form.tax_ids.clear()
        return invoice_form.save()   
        
        
        
#    def _create_invoice_from_xlsx(self, filename, tree):
#        print (tree)
#        temp_user_dict = {}
#        for h in tree.values.tolist():
#            date_test = h[1].strftime(DEFAULT_SERVER_DATE_FORMAT)
#            users_ids_tag = self.env['res.partner'].search([('category_id', '=', h[2])]).ids
#            code_product = [[o.id, o.name, o.code, o.default_code, o.barcode] for o in self.env['product.product'].search([]).filtered(lambda r: r.code == h[3])] # WORK
#            price = h[-1]
#            count = float(h[-2])
#            for b in users_ids_tag:
#                try:
#                    temp_user_dict[b].append([date_test, code_product[0], price, count])
#                except KeyError:
#                    temp_user_dict[b] = [[date_test, code_product[0], price, count]]
#        invoice = self.env['account.move']
#        invoice.move_type = "entry"
#        for U in temp_user_dict:
#            if temp_user_dict[U]:
#                with Form(invoice.with_context()) as invoice_form:
#                    self_ctx = self.with_company(invoice.company_id)
#                    invoice_form.invoice_date = temp_user_dict[U][0][0]
#                    invoice_form.partner_id = self.env['res.partner'].browse(U)
#                    for element in temp_user_dict[U]:
#                        with invoice_form.invoice_line_ids.new() as invoice_line_form:
#                            invoice_line_form.product_id = self_ctx._retrieve_product(
#                                                                                        default_code=element[1][-2],
#                                                                                        name=element[1][1],
...
#        product_work = self.env['product.product'] #self.env['product.template']
#        #client_tag = self.env['account.account.tag']
#        #client_tax = self.env['account.tax']
#            
##        ls = product_work.fields_get()
##        product_ = product_work.browse(35).name
##        for i in ls:
##            product_ = product_work.browse(35)[i]
##            if "tovar_001" == product_:
##                print (i, product_) #code tovar_001

#[[o.name, o.code] for o in product_work.sudo().search([('code', '=like', 'tovar_001')])]
#[o for o in product_work.sudo().search([('code', '=like', 'tovar_001')]).mapped('code')] #ERROR 
#product_work.search([]).filtered(lambda r: r.code == 'tovar_001').sorted(key=lambda r: r.code) # WORK
#[[o.id, o.name, o.code] for o in product_work.search([]).filtered(lambda r: r.code == 'tovar_001')] # WORK
